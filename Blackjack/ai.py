...
class Agent:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            trajectory = [] # stores all the states in order
            reward = {}
            state = self.simulator.state # initial state

            while state is not None:
                trajectory.append(state)
                reward[state] = self.simulator.check_reward()
                action = self.default_policy(state)
                state = self.make_one_transition(action)
            ...
            for i in range(size):
                ...
                for j in range(i, size):
                    dc_reward += (DISCOUNT ** degree) * reward[trajectory[j]]
                    degree += 1
                self.S_MC[trajectory[i]] += dc_reward
                self.N_MC[trajectory[i]] += 1
                
        for state in self.S_MC:
            if self.S_MC[state] == 0:
                self.MC_values[state] = 0
            else:
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            size = len(trajectory)
            for i in range(size - 1): # fill in the last state at the end manually
                state = trajectory[i]
                n_state = trajectory[i + 1]
                self.TD_values[state] = self.TD_values[state] + self.alpha(self.N_TD[state]) * (reward[state] + DISCOUNT * self.TD_values[n_state] - self.TD_values[state])
                self.N_TD[state] += 1

            self.TD_values[trajectory[size - 1]] = reward[trajectory[size - 1]]
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            n_state = self.make_one_transition(a) # initial next state
            if n_state is not None:
                n_a = self.pick_action(n_state, epsilon=0.4)

            while n_state is not None:
                self.Q_values[state][a] = self.Q_values[state][a] + self.alpha(self.N_Q[state][a]) * (reward + DISCOUNT * self.Q_values[n_state][n_a] - self.Q_values[state][a])
                self.N_Q[state][a] += 1

                state = n_state
                reward = self.simulator.check_reward()
                a = n_a
                n_state = self.make_one_transition(a)
                if n_state is not None:
                    n_a = self.pick_action(n_state, epsilon=0.4)

            self.Q_values[state][HIT] = self.simulator.check_reward()
            self.Q_values[state][STAND] = self.simulator.check_reward()
    ...
    def pick_action(self, s, epsilon):
        ...
        elif self.Q_values[s][HIT] > self.Q_values[s][STAND]:
            return HIT
        else:
            return STAND
